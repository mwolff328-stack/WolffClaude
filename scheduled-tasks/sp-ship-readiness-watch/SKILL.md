...
6. Gate exactly as the pre-deploy skill and sp-autonomous Phase 5 describe: `gh workflow run pre-publish.yml --ref 2026-v1` and `gh workflow run playwright-ci.yml --ref 2026-v1`. Verify `headSha` on each dispatched run equals the exact tip. Wait for both to complete (pre-publish.yml measured 27-47 min across 5 recent runs on 2026-08-24 — was estimated "15-25 minutes" here, corrected same day; playwright-ci.yml is faster, ~3-5 min — poll, don't guess or shortcut). Pull real executed/passed/skipped counts from the run logs, not just pass/fail colour — follow the pre-deploy skill's exact guidance on where each number lives (Stage 1 has no Summary line in the gate output; integ-core does).

7. Verify board status on Notion (SP Stories & Tasks database) for any SST-### tickets named in the new commits' messages. Use the OAuth Notion connector tools (`mcp__d77c6777-2678-446f-b1ea-d56a8303dfb6__notion-*`, e.g. `notion-query-data-sources` / `notion-fetch` — not `mcp__notionApi__*`) — confirmed working as of 2026-08-20, so the earlier "connector unavailable, use the browser" workaround no longer applies. Fall back to the Chrome browser tools (mcp__claude-in-chrome__*) only if the OAuth connector tools themselves error. Read the actual Status property on each ticket; never infer status from a commit message alone.
