...
function formatMessage(msg: DiscordMessage): string {
  ...
  const lines: string[] = [`[${time}]${edited} **${author}**: ${msg.content}`];
  if (msg.attachments.length > 0) {
    lines.push(`  Attachments: ${msg.attachments.map((a) => a.filename).join(", ")}`);
  }
  if (msg.embeds.length > 0) {
    for (const e of msg.embeds) {
      if (e.title) lines.push(`  Embed: ${e.title}${e.description ? ` — ${e.description}` : ""}`);
    }
  }
  return lines.join("\n");
}
...
server.registerTool(
  ...
  {
    description:
      "Fetch recent messages from a Discord channel. Returns messages in reverse-chronological order (newest first). Use channel_id from the Discord URL or get_channel_info.",
...
server.registerTool(
  ...
  {
    description:
      "Fetch messages from a Discord channel and filter by keyword. Searches through up to 100 recent messages (Discord's REST API doesn't support full-text search for bots). Returns matching messages with context.",
    inputSchema: z.object({
...
server.registerTool(
  ...
  async ({ channel_id }) => {
    const channel = (await discordFetch(`/channels/${channel_id}`)) as DiscordChannel;
...
server.registerTool(
  ...
  {
    description:
      "List all channels in a Discord server (guild). Useful for finding channel IDs by name.",
    inputSchema: z.object({
  ...
  async ({ guild_id }) => {
    ...
    const lines = sorted.map(
      (c) => `#${c.name ?? c.id} (${typeNames[c.type] ?? c.type}) — ID: ${c.id}`
    );
    return {
