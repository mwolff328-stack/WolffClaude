## Autonomous Development Workflow (generic default — overridden by a project Operating Model)

**For projects WITHOUT their own operating model**, prefer this pipeline: start with documentation, then plan, then implement with tests, rather than writing code directly.

Trivial non-code tasks (updating a comment, fixing a typo in docs) are exempt. Projects with an operating model follow that model's flow instead.
