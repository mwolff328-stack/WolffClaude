...
- [Proving a test is load-bearing](feedback_proving_a_test_is_load_bearing.md) — seven ways a RED proof gets faked; nothing proven until shown red against wrong code.
...
- [Verify a reviewer's evidence, not their judgement](feedback_verify_a_reviewers_evidence_not_their_judgement.md) — scope call was right, 2 of 5 supporting files were wrong.
...
- [Renaming for clarity can re-create the defect](feedback_renaming_for_clarity_can_recreate_the_defect.md) — a new name can lie on one branch, fail-open on its own subject.
...
- [One sampled error, many root causes](feedback_survivorpulse_one_sampled_error_many_root_causes.md) — 13 identical failures, two causes, two routes; grep each.
...
- [Collapse stale backlogs, don't maintain them](feedback_survivorpulse_collapse_stale_backlogs.md) — a groomed epic's rotted premises get ONE re-survey story, not upkeep.
...
- [A task premise may describe an unmerged sibling branch](feedback_task_premise_may_describe_an_unmerged_sibling_branch.md) — true elsewhere, false on your branch.
...
- [Rely on self-learning, not manual monitoring](feedback_survivorpulse_rely_on_self_learning_not_manual_monitoring.md) — the learnings queue + weekly review replace polling.
...
- [Derive from the quantity the reader validates](feedback_derive_from_the_quantity_the_reader_validates.md) — a field computed from a different quantity on write vs read drifts.
...
- [A bug ticket's Proposed resolution can carry the defect](feedback_a_bug_tickets_proposed_resolution_can_carry_the_defect.md) — "just reuse the helper" silently DELETED a cap.
...
- [A harness disagreement is evidence about the harness](feedback_a_harness_disagreement_is_evidence_about_the_harness.md) — shared test-arm state faked a 5-of-7 split.
- [Concurrent reviewer agents race on shared file reverts](feedback_concurrent_reviewer_agents_race_on_shared_file_reverts.md) — 3 subagents reverting the same file caused transient (harmless) collisions.
- [Verify a fix site is live before citing it](feedback_verify_fix_site_is_live_before_citing_it.md) — pool-form.tsx matched the bug but is wired only to the frozen V2 router.
- [A new error code activates old dead UI code](feedback_a_new_error_code_activates_old_dead_ui_code.md) — a 409 became reachable for a caller whose remediation UI was wired to a different track.

## Shipping, environments, and infra

- [Pre-publish gate is CI-only](project_survivorpulse_prepublish_gate_mechanism.md) — how to run the real gate, what each stage runs, publish-time secret deletion.
...
- [Replit deployment is autoscale](project_survivorpulse_replit_deployment_is_autoscale.md) — prod runs multiple instances; an in-process cache serves inconsistent data.
...
- [Production smoke access](project_survivorpulse_production_smoke_access.md) — prod IS reachable for read-only smoke; admin API is the only read path to prod data.
- [Deployed dev app URL](project_survivorpulse_deployed_dev_url.md) — founder-confirmed workspace preview URL; try it first, ask founder to refresh if stale.
...
- [SST-1247 residual-gap rulings](project_survivorpulse_sst1247_residual_gap_rulings.md) — off-season Home copy stays; zero-entry pool gets root-cause fix, routed Pam→Ann.
...
- [CA1 is self-contained](project_survivorpulse_ca1_is_self_contained.md) / [snapshot CRLF churn](project_survivorpulse_ca1_snapshot_crlf_churn.md) — no regen needed for scoring changes; a full suite shows golden .snap as modified with ZERO real change.
...
- [Hover shading over inline backgrounds](project_survivorpulse_hover_shade_over_inline_backgrounds.md) — class `:hover{background}` loses to inline styles; use an inset box-shadow.
...
- [Radix outside-click arming race](project_survivorpulse_radix_outside_click_arming_race.md) — armed via `setTimeout(…,0)`; an immediate click is dropped silently.
...
- [Dual app-entry trap](project_survivorpulse_dual_app_entry_trap.md) — prod compiles App-v1.tsx, dev uses App.tsx; a root hook added to one ships without it.
- [login ?next= is same-origin only](project_survivorpulse_login_next_param_is_same_origin_only.md) — wouter's pushState throws cross-origin; sign-in catch swallows it.
...
- [Sandbox has no local Postgres](project_survivorpulse_sandbox_has_no_local_postgres.md) — DB-integration RED/GREEN proof comes from a targeted `gh workflow run pre-publish.yml` dispatch, not a local run.
...
- [E2E CI drift traps](project_survivorpulse_e2e_ci_drift_traps.md) — a persistent E2E DB masked months of drift; shards share one DB, any spec can break the run.
...
- [tsc excludes the tests/ directory entirely](project_survivorpulse_tsc_excludes_tests_directory.md) — include is only client/src, shared, server; a compile-probe in tests/ never runs.
...
- [A fabricated finality tier splits the spread paths](project_survivorpulse_fabricated_finality_tier_splits_the_paths.md) — a fabricated closing stamp splits analytics from the UI.
- [Apply write-order collision](project_survivorpulse_apply_write_order_collision.md) / [past-season cascades](project_survivorpulse_past_season_apply_cascades.md) — cell-at-a-time writes make a MOVE collide with itself; completed weeks inherit kickoff-lock + elimination duties.
...
- [Reset-to-auto stays grid-matched, not Apply-scoped](project_survivorpulse_reset_to_auto_apply_scope_divergence.md) — founder ruling 2026-08-03; PoolSwitcher-as-scope already rejected once.

## App structure and data ownership

- [The Notion comment "outage" is connector-specific](project_survivorpulse_notion_comment_outage_is_connector_specific.md) — a second connector's create-comment works; stop falling back to Notes.
- [Notion via Chrome overwrites fields](project_survivorpulse_notion_via_chrome_field_overwrite.md) / [get-comments can't see replies](project_survivorpulse_notion_create_comment_write_path_defect.md) — ⚠️ NO write defect; get-comments is blind to replies in every mode tried.
...
- [Route auth is opt-in](project_survivorpulse_route_auth_is_opt_in.md) / [sp_session is a bearer token](project_survivorpulse_session_cookie_is_a_bearer_token.md) — missing middleware arg = anonymous-reachable; session has no IP/UA binding, 12h idle / 7d absolute.
...
- [Admin Real/Test pool marker](project_survivorpulse_admin_pool_classification.md) / [default-user-id trap](project_survivorpulse_default_user_id_ownership_trap.md) — `isTestData` arms hard-deletion; "View all pools"-only visibility means dev-stub ownership.
...
- [PickGrid.tsx is dead; SeasonGridSection/WeekViewSection are the shared implementation](project_survivorpulse_pickgrid_dead_seasongrid_shared.md) — Game Plan AND both My Pools surfaces render the same component instances.
- [My Strategy wizard: deleted 2026-08-03](project_survivorpulse_my_strategy_wizard_unreachable.md) — Step1,2,5 gone (SST-1261); Step3/4/StrategyWizardContext/StrategySelectionModal survive via /tools/*.
- [TanStack Query keys hash by value](project_survivorpulse_tanstack_query_keys_hash_by_value.md) — two hooks in different files with the same literal queryKey share ONE cache entry.
