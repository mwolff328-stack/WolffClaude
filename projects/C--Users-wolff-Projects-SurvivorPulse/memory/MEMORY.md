...
- [Replit credit exhaustion causes prod login failure](project_survivorpulse_replit_credit_exhaustion_causes_prod_login_failure.md) — generic "unexpected error" looked like a Neon endpoint issue; check Replit billing too.
...
- [Full-suite run needs TEST_DISABLE_NETWORK env](feedback_full_suite_run_needs_test_disable_network_env.md) — bare `npx vitest run client/src` hangs 30+ min against unreachable local Postgres; match pre-publish.yml's env vars.
