...
- [Await vs fire-and-forget for usage events](project_survivorpulse_usage_event_await_vs_fire_and_forget.md) — await only when the write IS the response payload.
...
- [Proving a test is load-bearing](feedback_proving_a_test_is_load_bearing.md) / [comment-only changes](feedback_proving_a_change_is_comment_only.md) — RED-proof and +/- filter both miss MOVES.
...
- [Guard the wire, not the helper](feedback_guard_the_wire_not_just_the_helper.md) / [wire-reached assertion](feedback_a_source_guard_must_assert_the_wire_is_reached.md) — 0/109 mutants killed; unreachable call read correct.
...
- [Enumerate a typed return's fields](feedback_enumerate_a_typed_returns_fields_before_signing_off.md) / [by structural anchor](feedback_enumerate_by_the_structural_anchor.md) — grep found 2/3; assertions found 1/2.
...
- [Source-text guards fooled by text](feedback_survivorpulse_source_text_guards_fooled_by_text.md) / [fail 4 ways](feedback_source_scanning_guards_need_three_meta_tests.md) — comments trip `toMatch()`; NUL hides the file.
...
- [Verify a deferral reason](feedback_survivorpulse_verify_a_deferral_reason.md) — never inherit a blocker from a peer.
...
- [Semantic dup guardrails git can't see](feedback_survivorpulse_semantic_duplicate_guardrails_git_cant_see.md) / [ledger timestamps unorderable](feedback_survivorpulse_claim_ledger_timestamps_are_not_orderable.md) — 2 sessions duplicated a ticket.
...
- [A value in output is not a constant](feedback_a_value_in_output_is_not_a_constant.md) — read the producer first.
...
- [jsdom normalizes inline styles](feedback_jsdom_normalises_inline_styles_two_traps.md) — drops a var() `border` shorthand; hex reads as rgb().
...
- [SST-1247 residual-gap rulings](project_survivorpulse_sst1247_residual_gap_rulings.md) — zero-entry pool fix RESOLVED (SST-1257+1258).
...
- [Bystander rule broke 2 distinctness tests](project_survivorpulse_bystander_rule_broke_two_distinctness_tests.md) — one flakes on a UUID coin flip.
...
- [openid-client PKCE broken in tests](project_survivorpulse_openid_client_pkce_broken_in_tests.md) — redirects to ?error= instead; stub one hash.
- [tsc excludes tests/ entirely](project_survivorpulse_tsc_excludes_tests_directory.md) — a compile-probe there never runs.
- [clearAllRateLimits() is cross-process no-op in Stage 2c](project_survivorpulse_clearallratelimits_is_cross_process_noop_in_stage2c.md) — compute real cross-file volume before trusting a ceiling.

## Engine and domain behaviour

- [SST-783 pseudo-replication reversal](project_survivorpulse_sst783_pseudo_replication_reversal.md) — reversed then RE-confirmed at 10 seasons; check current doc state.
- [SST-782 rank and scope errors](project_survivorpulse_sst782_rank_and_scope_errors.md) — "#5-#18 everywhere" wrong in 2/36 setups; wipeout table unscoped.
...
- [client/src/content/ is governance-scanned](project_survivorpulse_content_dir_governance_scan.md) — exports must be PLAIN DATA, not a RegExp or 1-arg function.
