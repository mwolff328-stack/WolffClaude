...
metadata: 
  ...
  modified: 2026-09-25T22:34:41.744Z
---
...
Update 2026-09-25 (afternoon): after the 1:33 PM reset the quota was fine: three genuine Cass runs in ~1.5 h (r17 14 min on the full 457K-char spec, r18 3.5 min, r19 ~3 min on a diff-scoped recheck). Give her the full spec once, then diff files (`r18-diff-*.txt`) for rechecks. Her r17/r18/r19 findings were all real and code-derived (opaque games.id vs semantic id; a season change stranding ids; emit-time pruning breaking a frozen rename; and a hole in the orchestrator's own R36 fix), so a fix round should always go back to genuine Cass, not just the persona reviewers. See [[bash-tool-quote-parse-failures-use-script-files]] for the prompt-file invocation.
