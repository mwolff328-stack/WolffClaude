...
metadata: 
  ...
  modified: 2026-09-19T05:49:21.455Z
---
...
- **Cross-time before/after on the shared dev box is noise** (same code: 1.7-7.7s within minutes; even control endpoints drift). Only a SAME-MOMENT A/B is valid: a temporary dev-only request flag selecting the old shape, pairs alternated within seconds, order flipped. Adding that flag was blocked by the permission classifier until the user said yes (it is gated on `ALLOW_UNSAFE_DEV_FEATURES`); ask first. The result reversed the earlier story: no gain on 5-entry pools, -20..-44% at 50 entries, so **test at the scale the change targets** (pad a demo pool to 50 entries via `POST /api/pools/:id/entries`, delete after; guard the delete).
- After a dev restart the auto-login user can change (ADMIN founder -> TEST_USER stub) and `ALLOW_UNSAFE_DEV_FEATURES` can be lost (401 on /api/me): needs the founder to restore the env AND restart the dev workflow.
- Real-DB fixtures: `picks.period` is NOT NULL DEFAULT 'week:1' and `storage.createPick` does not derive it from `week`; omit it and the pick lands below startingWeek and is floored out (CI-only failure, cost a 1.5h gate cycle).
