...
⚠️ This step is ONLY about the **Deployment secret** (founder ruling, 2026-07-24) — never touch a `.replit` `[userenv.development]` copy of this flag if one exists; that copy is dev-only (the deployment runs `npm run start` / `NODE_ENV=production` and does not read `[userenv.development]`), and removing it would disable the deployed dev app's auto-login.

**Drift note, found 2026-07-31: as of the current `2026-v1` HEAD, `.replit` `[userenv.development]` does NOT contain an `ALLOW_UNSAFE_DEV_FEATURES` line at all.** It was present and repo-committed as of `9c016764` (2026-07-22, re-enabled after a false-positive outage investigation — see that commit for the full incident) but was silently deleted, along with its explanatory comment block, by the Replit platform's own `60f7a897 "Published your App"` auto-commit on 2026-07-24 — the exact failure mode the deleted comment warned about (a value not committed to the repo gets wiped by Replit's sync), just from the opposite direction: the *deploy sync itself* pushed a config that had already lost the line. No commit since has restored it. This means either dev auto-login in the live Replit workspace is currently off, or it's back on via the Replit Configurations UI only and will be silently wiped again on the next publish. **The repo file cannot confirm live workspace state — only that the committed source of truth is missing this flag.** Before relying on dev auto-login (including for the `sp-live-verify` skill's §3), confirm live behavior against the deployed dev app directly (`GET /api/me` with no cookie: 200 = auto-login active, 401 = not), and if it's off and wanted back, re-add the line to `.replit` in the repo rather than only the Replit UI, or it will be lost on the next publish again.
